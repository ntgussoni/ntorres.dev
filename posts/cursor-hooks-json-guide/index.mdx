---
date: 06/07/2026
title: "Cursor hooks.json: the JSON Schema & Payload Reference"
description: "Every Cursor hooks.json event with its stdin JSON payload fields: afterFileEdit edits, exit codes, matchers, failClosed, and copy-paste recipes."
image: card.png
draft: false
categories:
  - ai
  ...
  - engineering
faq:
  - question: "Does Cursor have hooks?"
    answer: |
      Yes. Cursor shipped lifecycle hooks in 2024 and has iterated heavily since. As of 2026 it is the most mature editor-integrated hook system, covering Tab, Agent, workspace, and cloud agent lifecycles. Configure hooks in .cursor/hooks.json (project) or ~/.cursor/hooks.json (user).
  - question: "What is the hooks.json schema?"
    answer: |
      The current schema (version 1) keys events by name. Each event maps to an array of hook entries with fields like command, type, matcher, failClosed, timeout, and loop_limit. Project hooks live at .cursor/hooks.json relative to the repo root; user hooks live at ~/.cursor/hooks.json.
  - question: "How do I run tests after every file edit?"
    answer: |
      Use an afterFileEdit command hook that runs your test command, and keep it fail-open so a failing test does not block the agent's edit. The full recipe, with the stdin payload, the monorepo variant, and the postToolUse alternative that shows failures back to the agent, lives in the dedicated guide at ntorres.dev/blog/cursor-hooks-run-tests-after-file-edit
  - question: "What fields does the afterFileEdit stdin payload contain?"
    answer: |
      The afterFileEdit payload includes conversation_id, generation_id, hook_event_name, workspace_roots, file_path, and edits (an array of old_string/new_string pairs). Read it as JSON from stdin in your hook script.
  - question: "Can I use Claude Code hooks in Cursor?"
    answer: |
      Yes. Enable third-party skills in Cursor Settings and existing .claude/settings.json hooks are auto-mapped to Cursor event names. Event names map automatically (PreToolUse becomes preToolUse). For full control, maintain a native .cursor/hooks.json alongside Claude config.
  - question: "What is the difference between Cursor and Claude Code hooks?"
    answer: |
      Cursor uses a flat schema in .cursor/hooks.json where each event maps to an array of { command, matcher } entries. Claude Code nests matcher groups with a hooks array inside each group, stored in settings.json. Same idea, different JSON shapes.
---
...
| **Enterprise / team dashboard** | Enterprise admins configure hooks at [cursor.com/dashboard](https://cursor.com/dashboard/team-content?section=hooks). Synced to all team machines. |
| **Plugins** | Hooks can ship inside Cursor plugins from the marketplace. |
| **Reuse Claude Code hooks** | Enable third-party skills in Cursor Settings. Existing `.claude/settings.json` hooks are auto-mapped to Cursor event names ([third-party hooks docs](https://cursor.com/docs/reference/third-party-hooks)). |

Project hooks live at `.cursor/hooks.json` (committed to git). User hooks live at `~/.cursor/hooks.json` (machine-wide).

### Claude Code

Claude Code has **`/hooks`**, a slash command that opens a **read-only browser**. It lists configured hooks by event, shows matchers, commands, and source files. Per the [official guide](https://code.claude.com/docs/en/hooks-guide): "The `/hooks` menu is read-only. To add, modify, or remove hooks, edit your settings JSON directly or ask Claude to make the change."
...
- **Manual review** (watching every tool call)
...
Official docs: [cursor.com/docs/agent/hooks](https://cursor.com/docs/agent/hooks)
...
![Rules, Skills, and Hooks: only hooks can block agent actions](rules-skills-hooks.png)
...
- Project hooks: commands relative to repo root (`.cursor/hooks/my-hook.sh`)
- User hooks: commands relative to `~/.cursor/` (`./hooks/my-hook.sh`)
...
[Cloud agents](https://cursor.com/agents) run **project** hooks from `.cursor/hooks.json` in your repo. They do **not** see user-level `~/.cursor/hooks.json`. Command hooks work; prompt hooks do not (no auth wiring in the cloud VM). Events that do not fire in cloud agents: `sessionStart` and `sessionEnd` (session lifecycle is tied to the IDE session), `beforeMCPExecution` and `afterMCPExecution`, the Tab hooks (`beforeTabFileRead`, `afterTabFileEdit`), and `workspaceOpen`. Everything else runs in cloud agents, including `beforeSubmitPrompt`, `preCompact`, `stop`, `afterAgentResponse`, and `afterAgentThought`.
...
- `0` = success
- `2` = block (same as returning `permission: "deny"`)
...
Every command hook receives a JSON object on stdin. These are the key fields per event, based on the typed definitions in the [cursor-hooks npm package](https://github.com/johnlindquist/cursor-hooks) and Cursor's docs.

All events share four base fields:
...
| Event | stdin payload (key fields) | stdout response |
|-------|----------------------------|-----------------|
| `beforeShellExecution` | `command`, `cwd` | `{ "permission": "allow" \| "ask" \| "deny", "user_message", "agent_message" }` |
| `beforeMCPExecution` | `tool_name`, `tool_input`, `url`?, `command`? | same permission response |
| `beforeReadFile` | `file_path`, `content`, `attachments` | `{ "permission": "allow" \| "deny" }` |
| `beforeSubmitPrompt` | `prompt`, `attachments` | `{ "continue": true }` |
| `afterFileEdit` | `file_path`, `edits` (`{ old_string, new_string }[]`) | none (side-effect only) |
| `preToolUse` | tool name + input | permission response, plus `updated_input` to rewrite the call |
| `postToolUse` | tool name + input + output | `additional_context`, `updated_mcp_tool_output` |
| `subagentStart` / `subagentStop` | subagent type | `subagentStop` can return `followup_message` |
| `sessionStart` | session info | `additional_context` (fire-and-forget) |
| `stop` | `status` (`"completed"`, `"aborted"`, `"error"`) | none |

Blocking events (`beforeShellExecution`, `beforeMCPExecution`, `preToolUse`, `subagentStart`) can return `permission: "allow" | "ask" | "deny"` plus optional `user_message` and `agent_message`. Exit code `2` is equivalent to `permission: "deny"`.

---

## Hook recipes you can copy today

Each recipe includes the `hooks.json` entry and a hook script. Put scripts in `.cursor/hooks/` and `chmod +x` them. Recipes 1-5 are the essentials; 6-8 come from [official Cursor docs](https://cursor.com/docs/agent/hooks); 9 is the run-tests pattern.
...
Set `failClosed: true` so a crashed script blocks execution rather than silently passing through.
...
`afterFileEdit` hooks don't need to return JSON if they're side-effect only. The agent already wrote the file; you're post-processing it.
...
For production, swap the simple path matcher for a `gitleaks` or `trufflehog` scan. The hook pattern is the same: read stdin, return `permission: deny` if policy fails.
...
Add `audit/` to `.gitignore` if you don't want logs in version control. For team-wide audit trails, pipe to stdout and ship to your log aggregator from a wrapper script.
...
If you're building chained subagent workflows, `subagentStop` can also return a `followup_message` to continue the loop programmatically. Use `loop_limit` as a circuit breaker so a runaway chain doesn't burn your quota.
...
### 7. Gate `kubectl apply` to production namespaces
...
**`.cursor/hooks/kube_guard.py`:** (abbreviated; full version in [Cursor docs](https://cursor.com/docs/agent/hooks))
...
### 8. Inject context at session start
...
### 9. Run tests after every file edit

This one is requested often enough that it grew into its own guide, with the stdin payload deep dive, the monorepo variant, the version where the agent sees its own failures, and the Claude Code equivalent: [Cursor hooks: run tests after every file edit](/blog/cursor-hooks-run-tests-after-file-edit).

The short version: an `afterFileEdit` command hook that runs your test command and exits 0. Leave `failClosed` unset (fail-open) so a red test is visible without bricking the agent's session. Pair it with a `preToolUse` hook if you would rather gate *commits* on green tests.

### Claude Code equivalents

The same policies work in Claude Code with different event names and nested JSON. Examples from [code.claude.com/docs/en/hooks-guide](https://code.claude.com/docs/en/hooks-guide), and the [Claude Code hooks guide](/blog/claude-code-hooks-guide) covers the settings.json schema and every event in depth:
...
[Claude Code](https://code.claude.com/docs/en/hooks-guide) ships a full hook system for the CLI agent loop. The [official reference](https://code.claude.com/docs/en/hooks.md) documents events across the full lifecycle, including:
...
| Documented events | ~20 agent + Tab + workspace | Full lifecycle (see [reference](https://code.claude.com/docs/en/hooks.md)) |
...
- **Wrong path.** Project hooks use `.cursor/hooks/...` from repo root. User hooks use `./hooks/...` from `~/.cursor/`.
- **Missing shebang or execute bit.** Scripts need `#!/bin/bash` and `chmod +x`.
- **Missing dependencies.** If your script calls `jq`, verify it's on `$PATH` in the hook environment. Don't assume.
- **Matcher syntax.** JavaScript regex, not grep. Start without a matcher, confirm the hook fires, then add filtering.
- **Timeout.** Default is a few seconds. Long hooks get killed; a killed hook with `failClosed: true` blocks the agent.
- **Circular invocations.** A hook that shells out to `cursor` CLI can recurse. Use `loop_limit`.
...
1. **`beforeShellExecution`** with a dangerous-command blocklist (recipe 1)
2. **`afterFileEdit`** with your formatter (recipe 2)
3. **`beforeReadFile`** if you have secrets in the repo (recipe 3)
...
Hooks are also the enforcement layer inside unattended AI coding loops: a loop that runs tests, gates shell commands, and blocks secrets before going to sleep. See the [Loop Engineering series](/blog/loop-engineering) for the full picture of building those loops.
