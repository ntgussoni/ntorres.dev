...
faq:
  ...
  - question: "How do I run tests after every file edit?"
    answer: |
      Use an afterFileEdit command hook (or postToolUse in Claude Code) that runs your test command. Keep it fail-open: tests that fail should not block the agent's edit. See the "Run tests after every file edit" recipe in this guide.
...
The most requested hook pattern: make sure the agent's edits didn't break anything. Runs your test suite after every agent write, so failures surface immediately instead of at review time.
...
# Only run tests for files that affect behavior, not lockfiles or config
if echo "$file" | grep -qE '\.(lock|json|md)$' && [ "$file" != "package.json" ]; then
  exit 0
fi
...
Leave `failClosed` unset (fail-open): a failing test should be visible, but it shouldn't brick the agent's session. Pair with a `preToolUse` hook if you want to gate *commits* on green tests instead. In Claude Code, use `PostToolUse` with matcher `Edit\|Write` and the same script.

This recipe now has a standalone guide with the stdin payload deep-dive, monorepo, agent-context, and full-suite variants: [Cursor hooks: run tests after every file edit](/blog/cursor-hooks-run-tests-after-file-edit).

### Claude Code equivalents

The same policies work in Claude Code with different event names and nested JSON. Examples from [code.claude.com/docs/en/hooks-guide](https://code.claude.com/docs/en/hooks-guide):
